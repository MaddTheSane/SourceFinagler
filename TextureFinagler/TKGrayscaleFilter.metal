...
/// A Metal routine that computes a multiply effect.
///
/// The code looks up the source pixel in the sampler and then multiplies it by the value passed to the routine.
[[stitchable]]
float4 grayscaleFilter(coreimage::sampler Image, float redScale, float greenScale, float blueScale, float alphaScale) {
